...
// Agregar columnas nuevas si no existen (migraciones simples)
try { db.exec("ALTER TABLE conversations ADD COLUMN last_model TEXT"); } catch {}
try { db.exec("ALTER TABLE conversations ADD COLUMN client_summary TEXT"); } catch {}
try { db.exec("ALTER TABLE conversations ADD COLUMN read_status TEXT NOT NULL DEFAULT 'new'"); } catch {}
...
const stmtUpdateConvName = db.prepare<[string | null, number]>(
...
export function getOrCreateConversation(
  ...
  let conv = stmtGetConvByPhone.get(phone) as Conversation | undefined;
  if (!conv) {
    conv = stmtInsertConv.get(phone, name ?? null) as Conversation;
  } else if (name && conv.name !== name) {
    stmtUpdateConvName.run(name, conv.id);
    conv = { ...conv, name };
  }
  return conv;
}
...
const stmtSetMode = db.prepare<["AI" | "HUMAN", number]>(
...
interface SetConnectionStateArgs {
  status: "disconnected" | "qr" | "connecting" | "connected";
  qr_string?: string | null;
  phone?: string | null;
}

export function setConnectionState(args: SetConnectionStateArgs): void {
  const current = getConnectionState();

  const qr =
    args.qr_string !== undefined ? args.qr_string : current.qr_string;
...
export function createProduct(
  ...
  return db
    .prepare(
      "INSERT INTO products (name, price, description, image_base64) VALUES (?, ?, ?, ?) RETURNING *"
    )
...
export function updateProduct(
  id: number,
  data: { name?: string; price?: string; description?: string; image_base64?: string | null }
): void {
  const fields: string[] = [];
...
// Lee personalidad y productos desde la DB; si no hay nada guardado usa el archivo.

export function setReadStatus(id: number, status: "new" | "read"): void {
  db.prepare("UPDATE conversations SET read_status = ? WHERE id = ?").run(status, id);
}
